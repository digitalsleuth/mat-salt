...
echo "==> Uploading mat-salt-$TAG_NAME.tar.gz.sha256.asc"
curl -XPOST -H "Authorization: token ${GITHUB_ACCESS_TOKEN}" -H "Content-Type: text/plain" -q "https://uploads.github.com/repos/digitalsleuth/mat-salt/releases/${RELEASE_ID}/assets?name=mat-salt-${TAG_NAME}.tar.gz.sha256.asc" --data-binary @/tmp/mat-salt-$TAG_NAME.tar.gz.sha256.asc

echo "==> Uploading mat-salt-$TAG_NAME.tar.gz.asc"
curl -XPOST -H "Authorization: token ${GITHUB_ACCESS_TOKEN}" -H "Content-Type: text/plain" -q "https://uploads.github.com/repos/digitalsleuth/mat-salt/releases/${RELEASE_ID}/assets?name=mat-salt-${TAG_NAME}.tar.gz.asc" --data-binary @/tmp/mat-salt-$TAG_NAME.tar.gz.asc

rm /tmp/mat-salt-${TAG_NAME}.tar.gz
rm /tmp/mat-salt-$TAG_NAME.tar.gz.sha256
rm /tmp/mat-salt-$TAG_NAME.tar.gz.sha256.asc
rm /tmp/mat-salt-$TAG_NAME.tar.gz.asc
